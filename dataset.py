...
class ImageStandardizer(object):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.image_mean = np.zeros((3, 1))
        self.image_std = np.zeros((3, 1))
    ...
    def fit(self, X):
        ...
        self.image_mean[0] = mean[0]
        self.image_mean[1] = mean[1]
        self.image_mean[2] = mean[2]

        # self.image_mean[0] = np.mean(X[:,:,:,0])
        # self.image_mean[1] = np.mean(X[:,:,:,1])
        # self.image_mean[2] = np.mean(X[:,:,:,2])

        std = np.std(X, axis= (0, 1, 2))

        self.image_std[0] = std[0]
        self.image_std[1] = std[1]
        self.image_std[2] = std[2]
    ...
    def transform(self, X):
        ...
        # TODO: Complete this function
        # X_transformed =
        R_transformed = (X[:,:,:,0] - self.image_mean[0]) / self.image_std[0]
        G_transformed = (X[:,:,:,1] - self.image_mean[1]) / self.image_std[1]
        B_transformed = (X[:,:,:,2] - self.image_mean[2]) / self.image_std[2]
        
        X_transformed = np.stack((R_transformed, G_transformed, B_transformed), axis = -1)
